...
{: .info}
> Auto backups use the Borg backup adapter. Selective restore isn't available for Borg-based backups, but you can use **View Files** and copy specific files to your server instead.

## Browsing Backup Contents

Want to check what's inside a backup before restoring it? You can peek inside any backup without actually restoring anything. This works from both the Backups page and the File Manager.

{% tabs browse %}

{% tab browse From Backups Page %}

1. Click the **Browse** button on a backup.
2. The file manager opens showing the backup's directory structure.
...
1. Open the **File Manager** on your server.
2. Navigate to the **Backups** section (`.backups` directory).
3. Your manual and scheduled backups are listed with their name, size, and creation date.
...
| Lock / Unlock | Toggle deletion protection |
| Delete | Remove the backup (disabled if locked) |
...
| Selective restore | Yes | Yes |
| Quick restore | Yes | Yes |
| Copy to server | Yes | Yes |
