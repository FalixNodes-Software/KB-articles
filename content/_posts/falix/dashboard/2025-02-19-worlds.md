...
title: "Worlds"
tags: Server
permalink: falix/dashboard/general/worlds
...
Every world on your server shows up as its own card. At a glance, you can see the world name (with a link to the map viewer for Java worlds), which world is currently active (marked with a **Primary** badge), the edition it belongs to (Java, Bedrock, or Hytale), its directory path, and the action buttons you can use to manage it.
...
When you delete a world, its associated Nether and End dimension folders are automatically removed too -- you don't have to clean those up separately.

### Mass Deletion

Need to clear out several worlds at once? Select multiple worlds using the checkboxes on each card, then click **Delete** in the floating action bar. A confirmation dialog will show you everything you've selected and ask you to type "DELETE" before proceeding.
...
Datapacks let you customize world behavior -- things like custom recipes, loot tables, advancements, and more. Click **Datapacks** on a world card to manage them. Here's what you can do:

- **Browse** -- Follow links to external datapack sources to find new datapacks.
- **Upload** -- Drag and drop a `.zip` file to install a datapack directly.
- **Installed list** -- See all currently installed datapacks with their name, size, and a delete button for each.
- **File Manager** -- Jump straight to the datapacks folder in the file manager for manual management.

## World Settings

Click **Settings** on any world card to open the [World Settings](/falix/dashboard/general/world-settings) page for that world. From there, you can configure game rules, time, weather, spawning, and much more.
