...
title: "Mods"
tags: Server
permalink: falix/dashboard/general/mods
...
Type a mod name into the search bar and results will appear from both Modrinth and CurseForge at the same time. Duplicate entries are automatically merged using slug matching, so you will not see the same mod listed twice. Results are sorted by relevance by default, but you can change the sort order using the filters.
...
- **Installed Badge** -- A green checkmark appears if the mod is already on your server
...
The dashboard automatically detects which mods are already installed on your server by matching filenames in your `/mods` directory against known mod slugs. Any mod that is already on your server will show a green **Installed** badge, so you can tell at a glance what you have and what you still need.
