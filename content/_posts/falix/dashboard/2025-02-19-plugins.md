...
description: "Browse, search, and install server plugins from Spigot, Modrinth, Hangar, and more."
keywords:
    - keyword: plugin
...
{: .info}
> The Plugins page is available for **Minecraft Java**, **Minecraft Bedrock**, and **PocketMine** servers.
...
Bedrock servers have a source selector at the top:
...
- Only plugins with **100+ downloads** are shown in search results. This filters out very new or untested plugins.
- Certain categories of resource-heavy or potentially abusive plugins are blocked, including:
  - Fake player and MOTD manipulation plugins
  - World pre-generation plugins (Chunky, ChunkMaster, etc.)
  - Scripting engines (Skript, Denizen, etc.)
  - Dynamic map plugins (Dynmap, BlueMap, SquareMap)

Upgrading to Premium removes all of these restrictions, giving you full access to every plugin.

## Installed Detection

The dashboard automatically detects which plugins are already installed on your server by matching filenames in your `/plugins` folder against known plugin slugs. Any plugin that is already on your server will display a green **Installed** badge, so you do not accidentally install the same thing twice.
