...
Minecraft servers support only one active resource pack at a time. The FalixNodes panel also allows uploading only a single pack. If you need features from more than one pack, you have two options:

### Use a Plugin (Recommended)

Plugins such as ItemsAdder (`paid`), Oraxen (`paid`), or ResourcePackManager can merge and manage multiple packs automatically.
These tools generate a combined pack and handle conflicts, hosting, and updates for you.

### Manually Combining Resource Packs

If you do not want to use plugins, you can combine resource packs into a single `.zip` file.

1. Extracting both packs.

2. Merging their `assets/` directories.

3. Resolving file conflicts (textures, models, lang files, etc.).

4. Re-zipping the final merged pack.

Tools like [Elmakers's Merge Tool](https://merge.elmakers.com/) can help automate this process.
